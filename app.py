...
def analyze(code_raw, period):
    ...
    try:
        ...
        sname=get_name(code_c) if code_c.isdigit() else ""
        row={'_code':code_c,'_ticker':ticker,'_sname':sname,
...
if run and st.session_state.wl:
  ...
  <div class="ma-row">{chip('MA5',r['_close'],r['_m5'])}{chip('MA10',r['_close'],r['_m10'])}{chip('MA20',r['_close'],r['_m20'])}</div>
        ...
        for r in results:
            ...
            rh+=f"<tr><td><b style='color:#58a6ff'>{r['代碼']}</b></td><td style='color:#8b949e'>{r['名稱']}</td><td><b>{r['最新收盤']}</b></td><td>{r['MA5']}</td><td>{r['MA10']}</td><td>{r['MA20']}</td>{td(r['vs MA5'])}{td(r['vs MA10'])}{td(r['vs MA20'])}</tr>"
...
<span style="color:#8b949e;font-size:.88rem;margin-left:8px">{name}</span>
</div>""", unsafe_allow_html=True)
else:
